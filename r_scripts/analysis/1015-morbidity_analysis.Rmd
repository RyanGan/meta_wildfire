...
# simple binary exposure mod
mod <- glm(hf ~ smoke_wave + temp_f + month + offset(log(population)), 
           family="quasipoisson"(link="log"), data = chelan)
summary(mod)
...
```{r}
binary_results <- read_csv(paste0("../../data/health/",
  "1015-ts_binary_smoke_results.csv")) %>% 
    mutate(outcome = forcats::fct_relevel(outcome,
    unique(binary_results$outcome)),
    group = as.factor(if_else(outcome %in% outcomes[1:5], 
                    "respiratory", "cardiovascular"))) 


ggplot(data=binary_results, aes(x=outcome, y = rr, colour = group)) +
  geom_point() +
  ...
  ylab("Risk Ratio") +
  xlab("Outcome") +
  ggtitle(paste0("Binary Smoke Association")) +
  ryan_theme +
