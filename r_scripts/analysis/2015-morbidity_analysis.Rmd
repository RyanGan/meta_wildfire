---
title: 'Wildfire Smoke and Morbidity 2015'
author: "Ryan Gan"
output:
  html_document:
    df_print: paged
...
Importing Colorado 2010 to 2015 time series.
...
Overall, the rate for cardiovascular outcomes does not exhibit the same seasonal trends as respiratory outcomes, and remains relatively stable over the period. The rates are similar between the two states. Before, I had said it looks almost 50% higher for Washington; but I believe this was because I was double-counting unique observations that appeared in both the inpatient and observation CHARS datasets. I've only included inpatient observations to match Colorado now.
...
ggplot(data=results_plot, aes(x=outcome, y = rr, colour = group)) +
  ...
  ylab("Risk Ratio") +
  xlab("Outcome") +
  ggtitle(paste0("Sensitivity: August Only")) +
  ryan_theme +
  theme(panel.grid.major.x = element_blank(),
        panel.grid.major.y = element_line(linetype = "dotted"),
        panel.grid.minor = element_blank(),
...
ggplot(data=results_plot, aes(x=outcome, y = rr, colour = group)) +
  ...
  ylab("Risk Ratio") +
  xlab("Outcome") +
  ggtitle(paste0("Sensitivity: Spokane")) +
  ryan_theme +
  theme(panel.grid.major.x = element_blank(),
        panel.grid.major.y = element_line(linetype = "dotted"),
        panel.grid.minor = element_blank(),
...
knitr::kable(pm_aic_tab, caption="Degree of Freedom for PM2.5 Smoke trend")
```
...
This code chunk takes forever. I've saved the best-fit df as a dataframe and turned off evaluation of this code chunk.
...
Spline best-fit table.

```{r dl_fit_tab}
# manual input of best df
lag_df <- c(3,4,5,2,2,3,5,3,2,2,2)
min_aic <- data_frame(outcomes, lag_df) %>% 
...
Running distributed_lag function on vector of each outcome. I'm going to save the results to a flat file so the knitting process for the markdown file is faster. This step is time-consuming.
...
Finding new distributed lag spline fits. First fitting new PM~2.5~ distributed lag fit on subsetted dataframe.
...
Finding spline degrees of freedom best fit of distributed lag.
...
Distributed lag fit table for sensitivity analysis.

```{r dl_fit_sn_tab}
# manual input of best df
lag_df <- c(3,4,5,2,2,3,5,3,2,2,2)
min_aic_sn <- data_frame(outcomes, lag_df) %>% 
  rename(outcome = outcomes)
...
Running sensitivity results based on new degrees of freedom.I've saved the results as a new .csv flat file so I don't have to run these each time as it slows down knitting of the markdown file.

```{r dl_sn_results, eval = F}
# defining exposure spline
pm_spl_sn <-ns(ed_lag_sn$date, df = 13)
# create outcome vector and degrees of freedom vector to use map2 function
outcome_vector <- min_aic_sn$outcomes
...
# save results so I don't have to run them everytime i want to knit doc
write_csv(dl_sn_results, "../../data/health/2015-morbidity_dl_sen_results.csv")
```

Reading back in the saved distributed lag sensitivity analysis results.

```{r read_dl_sn_results}
dl_sn_results <- read_csv(paste0("../../data/health/",
  "2015-morbidity_dl_sen_results.csv")) %>% 
  mutate(outcome = forcats::fct_relevel(outcome, outcomes))
```


Sensitivity cumulative effect results. Results do not change much compared to full time series.
...
Sensitivity lagged results. Results do not change much from full time sereis.
...
It looks like there may be a cumulative effect on respiratory outcomes, all respiratory, asthma, and COPD following 3 to 4 days of smoke exposure. There may also be a signal with cardiovascular disease. Sensitivity resutls based on time frame are similar, but I've seen some different results that I'm not confident of with some of the classifications of CVD (MI in particular) when I vary the degrees of freedom for the exposure spline or distributed lag spline.

I still have some concerns that the signal with asthma is not as pronounced on the same day as our previous studies. I tend to use this as a marker.

I'm going to run these same analyses using a time-stratified case-crossover approach instead of a time-series.

## Note for Jeff: 
Regarding the Colorado sensitivity analysis of only including the front range counties. I've done this and find the results don't differ much when including the other counties outside the front range. I think the front range counties carry the most weight as they have the most numbers of events. However, I will take note of this and keep an eye out on further analyses.

## Next Steps:
- I plan to run time-stratified case-crossover analyses on 2015 data as well. 
- I will also use the kriging data Kate provided that covers 2011-2015 with available data, which will provide a larger dataset to evaluate. 
- Plan to run strata-specific results; consider incorporating cesus covariates for SES.
- Work in 2012 Washington and 2013 Oregon data.
