...
With less degrees of freedom, the spline fit is less pronounced. Even at the higher degrees of freedom, the major smoke event doesn't have much of an influence, but you can see the line influenced slightly. I think I might actually try and fit a spline with 5 degrees of freedom to the following models for a couple reasons. I want to account for the natural variation in the exposure series that may also confound the outcome series. Furthermore, I don't think I actually want to capture the variation of the smoke events as this is what we are interested in. In other words, I think I am trying to account for what would have been a natural pattern if the smoke were not present. This also raises a question about the GWR Smoke variable, which subtracts off the median value over this time. I wonder if that's appropriate? Question to think about.

Fitting univariate model with 7 degrees of freedom spline

```{r results_pm_spline}
# define pm spline
pm_spl <- ns(total_ed$date, df = 7)
...
distributed_lag <- function(data, exp_mat, outcome, 
  ...
  # estimate cumulative if true
    if(cumulative==T) {
      ...
      # assign column names
      colnames(return_estimate) <- c("type","time","estimate",
                                     "lower95","upper95") 
      return(return_estimate)
      # if cumulative is not true, or false, return distributed lag
    } else {  
...
result2 <- distributed_lag(data=ed_ts_lag, exp_mat = smk_matrix, 
                          outcome = "asthma", lag_df = 3, pm_spline = pm_spl,
                          cumulative = F)
...
```{r cumulative_results}
cumulative_results <- outcomes %>% 
  map_dfr(distributed_lag(data=ed_ts_lag, exp_mat = smk_matrix, 
                          outcome = ., lag_df = 3, pm_spline = pm_spl,
                          cumulative = F))
