...
summary(glmer(hf_n ~ smoke10 + smk_lag1 + smk_lag2 + smk_lag3 + smk_lag4 +
             smk_lag5 + smk_lag6 + smk_lag7 + (1|day) + (1|county) + (1|state) + 
             offset(log(population)), meta_ts2,
             family="poisson"))

# continous pm
mixed_mod <- glmer(hf_n ~ geo_smk10 + geo_smk_lag1 + geo_smk_lag2 + 
...
```{r two stage dlnm setup, message = F, warning=F}
...
```{r two stage distributed lag loop, message=F, warning=F, results="asis"}
...
for(i in seq(county_list_df)) {
  ...
  county_model <- glm(asthma_n ~ smk_cb + #temp_cb + 
                        offset(log(population)), 
...
# predictor-specific summaries for 10 ug/m^3 of smoke
pooled_cp <- crosspred(blag, coef=coef(cumulative_mv_meta), 
  vcov=vcov(cumulative_mv_meta), model.link="log", from = 0, to = 7, at = 1,  cumul = TRUE)

# i'd eventually like to figure out how to use the estimates to calculate by hand
str(pooled_cp)
?crosspred
pooled_cp$coefficients
...
# vector of exponentiated specific associations
pooled_cp$matRRfit

# vectors of cumulative association
pooled_cp$allRRfit
pooled_cp$allRRlow
pooled_cp$allRRhigh
...
# use qaic to evaluate model fit
sum(qaic_val)
