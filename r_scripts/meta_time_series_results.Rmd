...
meta_ts2 <- meta_ts %>%
  ...
  # binary smoke lag >15
  mutate(smk_lag1 = lag(smoke10, 1, order_by = state_county),
         smk_lag2 = lag(smoke10, 2, order_by = state_county),
         smk_lag3 = lag(smoke10, 3, order_by = state_county),
         smk_lag4 = lag(smoke10, 4, order_by = state_county),
         smk_lag5 = lag(smoke10, 5, order_by = state_county),
         smk_lag6 = lag(smoke10, 6, order_by = state_county),
         smk_lag7 = lag(smoke10, 7, order_by = state_county),
         smk_lag8 = lag(smoke10, 8, order_by = state_county),
         smk_lag9 = lag(smoke10, 9, order_by = state_county),
        smk_lag10 = lag(smoke10, 10, order_by = state_county))
  

# simple distributed lag models

dis_lag_mod <- glmer(asthma_n ~ smoke10 +
  wrf_temp + (1|day) + (1|county) + (1|state) +  
  offset(log(population)), meta_ts, family="poisson")

summary(dis_lag_mod)

summary(glmer(asthma_n ~ smoke10 + smk_lag1 + smk_lag2 + smk_lag3 + smk_lag4 +
             smk_lag5 + smk_lag6 + smk_lag7 + (1|day) + (1|county) + (1|state) + 
             offset(log(population)), meta_ts2,
             family="poisson"))

# continous pm
mixed_mod <- glmer(asthma_n ~ geo_smk_pm + geo_smk_lag1 + geo_smk_lag2 + 
  geo_smk_lag3 + geo_smk_lag4 + geo_smk_lag5 + geo_smk_lag6 + geo_smk_lag7 +
  (1|day) + (1|county) + (1|state) + offset(log(population)), meta_ts2,
...
counties <- as.character(unique(meta_ts2$state_county))

# create dataframe list of each county
county_list_df <- lapply(counties, function(x) 
  meta_ts2[meta_ts2$state_county==x,])
...
# attempting dl model with asthma
...
for(i in seq(county_list_df)) {
  ...
  suppressWarnings({
  smk_cb <- crossbasis(sub$geo_smk10,
  ...
  asthma_model <- glm(asthma_n ~ smk_cb + temp_cb + 
                    offset(log(population)), 
  ...
  suppressWarnings({
    asthma_cr_all <- crossreduce(smk_cb, asthma_model, type = "var", value = 1)
  ...
  # Q-AIC
  qaic_asthma[i] <- fqaic(asthma_model)
...
asthma_county_all <- apply(asthma_all,1, function(x) exp(blag%*%x))

# prediction for a grid of lag days

# predictor-specific summaries for 10 ug/m^3 of smoke
asthma_cp_smk10 <- crosspred(blag, coef=coef(mv_asthma_all),
                      vcov=vcov(mv_asthma_all),
                      model.link="log", at = 0:70/10)

# # summary stats for resp
asthma_cp_smk10$allRRfit
asthma_cp_smk10$allRRlow
asthma_cp_smk10$allRRhigh

# plot of asthma
plot.crosspred(asthma_cp_smk10, type="n", ylim=c(.8, 1.2), ylab="RR", xlab="Lag")
#matplot(xlag, resp_county_all, type="l", col=grey(0.5),lty=2,add=T)
abline(h=1)
lines(asthma_cp_smk10,col=2,lwd=2)
legend ("top","Pooled (with 95%CI)",#,"First-stage region-specific"),
  lty=c(1,2),lwd=1.5,col=c(2,grey(0.7)),bty="n",inset=0.1,cex=0.8)
mtext(text=paste("10 ug/m^3 increase in GWR Smoke and Asthma ER"))
