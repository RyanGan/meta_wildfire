...
# fit mixed model
mod <- glmer(asthma_n ~  pm_b + wrf_temp + (1|state_county) + offset(log(population)), meta_ts2, family="poisson")

# output distributed lag beta parameters
dlparms <- mod@beta[2:5]
dl_estimates <- data.frame(estimate = b %*% dlparms)
exp(dl_estimates)
exp(sum(dl_estimates))

str(mod)

vcov(mod)
# estimate distributed lag values
dl_estimates <- data.frame(estimate = b %*% dlparms)
# estimate variance matrix
dl_var <- b %*% vcov(mod)[dlparms,dlparms]
