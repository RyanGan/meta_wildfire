...
fit_mat <- matrix(NA, nrow = 3, ncol = 6) 
  ...
  for(h in 3:5){ # start loop for df
    ...
    for(i in seq(county_list_df)) {
      ...
      if(max(sub[, which(colnames(sub)==outcome_names[[q]])]) > 1){
    ...
    fit_mat[h-2,5] <- round(sum(qaic_val[converge[]==T]),2)
  ...
  # this part can be moved until df is IDed-----
  # second stage ----
...
# I can comment this out until the degrees of freedom is decided
