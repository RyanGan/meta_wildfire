...
### Scatter Plot of Rate and Geo Smoke

```{r scatter plot, message=F, warning=F, results="asis"}
ggplot(meta_ts, aes(x = wrf_smk_pm, y = log(asthma_n/population))) +
...
```{r crude analysis, message=F warning=F, results="asis"}
glimpse(meta_ts)

quasipois_mod <- glmer(asthma_n ~ geo_wt_pm + wrf_temp + 
                     (1|county) + (1 | state) +
