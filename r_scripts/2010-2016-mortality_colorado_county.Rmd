...
fips_path <- paste0("../colorado_wildfire/",
  "data/shapefiles/colorado_county_fips.csv")

colorado_fips <- read_csv(fips_path, col_types = cols(.default = "c")) %>% 
  # remove var
  select(-var) %>% 
  mutate(state_fips = paste0("0", state_fips),
         county_fips = stringr::str_sub(paste0("00", county_fips), 
                                        start = -3),
...
```{r write_data}
write_path <- paste0("./data/health/2010-2016_mortality_co_ts.csv")
